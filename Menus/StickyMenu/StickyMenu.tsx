...
const StickMenu = () => {
  ...
  const getBlueBarPosition = () => {
    if(titleRef.current && menuRef.current) {
      const positionTop = titleRef.current.offsetTop  - menuRef.current.clientHeight + (titleRef.current.clientHeight) + 10
      const positionLeft = titleRef.current.offsetLeft - (titleRef.current.clientWidth / 4)
      const maxWidth = titleRef.current.clientWidth + (titleRef.current.clientWidth / 2)
      setPositon({
